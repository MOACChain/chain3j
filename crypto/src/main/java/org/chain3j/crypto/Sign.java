...
public class Sign {
     ...
     * @return An ECKey containing only the public part, or null if recovery wasn't possible.
     */
    public static BigInteger recoverFromEIP155Signature(int recId, ECDSASignature sig, byte[] message) {
        verifyPrecondition(recId >= 0, "recId must be positive");
        verifyPrecondition(sig.r.signum() >= 0, "r must be positive");
        verifyPrecondition(sig.s.signum() >= 0, "s must be positive");
        verifyPrecondition(message != null, "message cannot be null");
     ...
     *     signature format error.
     */
    public static BigInteger signedEIP155MessageToKey(
            byte[] message, SignatureData signatureData) throws SignatureException {
        ...
        byte[] messageHash = Hash.sha3(message);
        int recId = header - 27;
        BigInteger key = recoverFromEIP155Signature(recId, sig, messageHash);
        if (key == null) {
