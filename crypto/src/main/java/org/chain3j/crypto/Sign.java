...
public class Sign {
     ...
     *
     * @param chainId The chainId of the network this signature will be used.
     * @param message Hash of the data that was signed.
     * @param keyPair An ECKey pair used to sign the message with chainId.
     * @return A signature with chainId in the headerByte.
     */
    public static SignatureData signEIP155Message(int chainId, byte[] message, ECKeyPair keyPair) {
        SignatureData sig = signMessage(message, keyPair, true);

        // For EIP155, the v = CHAIN_ID * 2 + 35 or v = CHAIN_ID * 2 + 36
        // instead of earlier signature scheme using v = 27 and v = 28
        int newV = sig.getV() + 8 + chainId + chainId;

        return new SignatureData((byte)newV, sig.getR(), sig.getS());
    }
     ...
     * public key to determine if the signature was correct, only used without EIP155 sign.
     *
     ...
     * with EIP155 rule,
     * returns the public key that was used to sign it. This can then be compared to the expected
     * public key to determine if the signature was correct, used with EIP155 sign,
     * which has a chainID at the end of the message.
     *
     * @param chainId integer as chainID, used to remove the header of the signature, V
     * @param message RLP encoded EIP 155 signed message.
     * @param signatureData The message signature components
     * @return the public key used to sign the message
     * @throws SignatureException If the public key could not be recovered or if there was a
     *     signature format error.
     */
    public static BigInteger signedEIP155MessageToKey(int chainId,
            byte[] message, SignatureData signatureData) throws SignatureException {
        ...
        // chainId should less than 114, otherwise header will be too large
        verifyPrecondition(chainId > 0 && chainId < 114, "chainId must be negative number");

        // Remove the chainId from header, and sub 8 to follow the algorithm
        // in VNODE transaction_signing.go
        // the header byte sub chainId * 2 and 8
        int header = signatureData.getV() & 0xFF;
        header = header - chainId - chainId - 8;
        ...
        BigInteger key = recoverFromSignature(recId, sig, messageHash);
        if (key == null) {
            throw new SignatureException("Could not recover public key from EIP155 signature");
        }
        return key;
    }
